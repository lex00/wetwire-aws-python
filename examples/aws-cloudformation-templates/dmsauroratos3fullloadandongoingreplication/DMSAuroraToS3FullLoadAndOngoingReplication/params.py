...
class ExistsDMSVPCRole(Parameter):
...
class ExistsDMSCloudwatchRole(Parameter):
...
class SnapshotIdentifier(Parameter):
...
class NotExistsDMSVPCRoleCondition(TemplateCondition):
    logical_id = 'NotExistsDMSVPCRole'
    expression = Equals(ExistsDMSVPCRole, 'N')


class NotExistsDMSCloudwatchRoleCondition(TemplateCondition):

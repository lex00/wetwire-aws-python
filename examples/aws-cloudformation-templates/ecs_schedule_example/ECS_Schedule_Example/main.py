...
class TaskDefinitionLogConfiguration(ecs.TaskDefinition.LogConfiguration):
    log_driver = 'awslogs'
    options = {
...
class TaskDefinitionMountPoint(ecs.TaskDefinition.MountPoint):
...
class TaskDefinitionLogConfiguration1(ecs.TaskDefinition.LogConfiguration):
    log_driver = 'awslogs'
    options = {
        'awslogs-group': CloudwatchLogsGroup,
        'awslogs-region': AWS_REGION,
...
class TaskDefinitionVolumeFrom(ecs.TaskDefinition.VolumeFrom):
    source_container = 'simple-app'


class TaskDefinitionContainerDefinition1(ecs.TaskDefinition.ContainerDefinition):
    name = 'busybox'
    cpu = 10
...
class TaskDefinitionKeyValuePair(ecs.TaskDefinition.KeyValuePair):
    name = 'my-vol'
...
class ServiceScalingPolicyStepAdjustment(applicationautoscaling.ScalingPolicy.StepAdjustment):
    metric_interval_lower_bound = 0
    scaling_adjustment = 200


class ServiceScalingPolicyStepScalingPolicyConfiguration(applicationautoscaling.ScalingPolicy.StepScalingPolicyConfiguration):

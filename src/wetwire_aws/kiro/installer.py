...
ALWAYS use the wetwire_init MCP tool to create new packages. DO NOT manually create __init__.py.

After running wetwire_init, add resource files:
```
my_package/
├── __init__.py      # GENERATED BY wetwire_init - DO NOT EDIT
├── storage.py       # S3, DynamoDB resources
├── compute.py       # Lambda, EC2 resources
└── security.py      # IAM roles, policies
```

RULES:
- NEVER edit __init__.py - it is auto-generated by wetwire_init
- NEVER put resource classes in __init__.py
- ALWAYS create separate .py files for resources (storage.py, main.py, etc.)
...
3. INIT - Use wetwire_init MCP tool to create package structure
4. IMPLEMENT - Write Python classes in separate module files (NOT __init__.py)
5. LINT - Run wetwire_lint MCP tool to check code patterns
6. BUILD - Run wetwire_build MCP tool to generate CloudFormation template
7. ITERATE - Fix any issues and rebuild

...
1. ALWAYS run wetwire_lint BEFORE wetwire_build
2. NEVER skip lint - even if you're confident the code is correct
3. Fix all lint errors before attempting to build
4. After EVERY code change, run lint immediately
...
├── params.py        # Create this file for Parameters
├── outputs.py       # Create this file for Outputs
...
1. PARAMETERS - Define in params.py, extend Parameter class:
   ```python
...
2. OUTPUTS - Define in outputs.py, extend Output class:
   ```python
   from . import *  # noqa: F403
   ...
   class BucketArn(Output):
       ...
       value = MyBucket.Arn
       export_name = "MyBucketArn"  # Optional
   ```

3. RESOURCE DECLARATION - In storage.py/compute.py, reference params:
   ```python
   from . import *  # noqa: F403

   class MyBucket(s3.Bucket):
       bucket_name = BucketName  # Reference parameter class directly
   ```

4. DIRECT REFERENCES - Reference other resources by class name:
   ```python
   class MyFunction(lambda_.Function):
       role = MyRole.Arn  # GetAtt via attribute access
   ```

5. TYPE-SAFE CONSTANTS - Use typed enums instead of strings:
   ```python
   runtime = lambda_.Runtime.PYTHON3_12  # Not "python3.12"
   ```

6. NESTED PROPERTY TYPES - Use wrapper classes for complex properties:
   ```python
   # WRONG - inline dicts fail:
   class MyBucket(s3.Bucket):
       bucket_encryption = {"Rules": [...]}  # FAILS at build

   # CORRECT - use wrapper classes with Service.Resource.PropertyType pattern:
   class MySSEDefault(s3.Bucket.ServerSideEncryptionByDefault):
       sse_algorithm = "AES256"

   class MySSERule(s3.Bucket.ServerSideEncryptionRule):
       server_side_encryption_by_default = MySSEDefault

   class MyEncryption(s3.Bucket.BucketEncryption):
       server_side_encryption_configuration = [MySSERule]

   class MyBucket(s3.Bucket):
       bucket_encryption = MyEncryption  # WORKS
   ```
   Key pattern: `<Service>.<Resource>.<PropertyType>` (e.g., `s3.Bucket.BucketEncryption`)
...
1. Run wetwire_lint immediately
2. Fix any errors it reports
3. Repeat until lint passes
4. ONLY THEN run wetwire_build
...
4. If lint passes, run wetwire_build to generate template:
...
- NEVER skip the lint step - it catches errors before build
...
When modifying code:
1. Edit the file
2. Run wetwire_lint IMMEDIATELY
3. Fix any errors before moving on
4. Only run wetwire_build after lint passes""",
}
    ...
    try:
        ...
        result = subprocess.run(
            [
                sys.executable,
                "-c",
                "import shutil; print(shutil.which('wetwire-aws-mcp') or '')",
            ],
            capture_output=True,
            text=True,
            timeout=10,
...
def launch_kiro(prompt: str | None = None, project_dir: Path | None = None) -> int:
    ...
    initial_message = (
        prompt if prompt else "Hello! I'm ready to design some AWS infrastructure."
    )

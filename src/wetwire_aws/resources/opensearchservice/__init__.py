...
class Application(CloudFormationResource):
    ...
    app_configs: list[DslValue[_Application.AppConfig]] = field(default_factory=list)
    data_sources: list[DslValue[_Application.DataSource]] = field(default_factory=list)
    endpoint: DslValue[str] | None = None
    iam_identity_center_options: (
        DslValue[_Application.IamIdentityCenterOptions] | None
    ) = None
    ...
    AppConfig = PropertyTypeDescriptor(_Application.AppConfig, "AppConfig")
    DataSource = PropertyTypeDescriptor(_Application.DataSource, "DataSource")
    IamIdentityCenterOptions = PropertyTypeDescriptor(
...
class Domain(CloudFormationResource):
    ...
    access_policies: DslValue[dict[str, Any]] | None = None
    advanced_options: dict[str, DslValue[str]] = field(default_factory=dict)
    advanced_security_options: DslValue[_Domain.AdvancedSecurityOptionsInput] | None = (
        None
    )
    aiml_options: DslValue[_Domain.AIMLOptions] | None = None
    cluster_config: DslValue[_Domain.ClusterConfig] | None = None
    cognito_options: DslValue[_Domain.CognitoOptions] | None = None
    domain_endpoint_options: DslValue[_Domain.DomainEndpointOptions] | None = None
    domain_name: DslValue[str] | None = None
    ebs_options: DslValue[_Domain.EBSOptions] | None = None
    encryption_at_rest_options: DslValue[_Domain.EncryptionAtRestOptions] | None = None
    engine_version: DslValue[str] | None = None
    identity_center_options: DslValue[_Domain.IdentityCenterOptions] | None = None
    ip_address_type: DslValue[str] | None = None
    log_publishing_options: dict[str, DslValue[_Domain.LogPublishingOption]] = field(
        default_factory=dict
    )
    node_to_node_encryption_options: (
        DslValue[_Domain.NodeToNodeEncryptionOptions] | None
    ) = None
    off_peak_window_options: DslValue[_Domain.OffPeakWindowOptions] | None = None
    skip_shard_migration_wait: DslValue[bool] | None = None
    snapshot_options: DslValue[_Domain.SnapshotOptions] | None = None
    software_update_options: DslValue[_Domain.SoftwareUpdateOptions] | None = None
    tags: list[DslValue[Tag]] = field(default_factory=list)
    vpc_options: DslValue[_Domain.VPCOptions] | None = None
    ...
    # PropertyType descriptors for nested GetAtt support
    AIMLOptions = PropertyTypeDescriptor(_Domain.AIMLOptions, "AIMLOptions")
    AdvancedSecurityOptionsInput = PropertyTypeDescriptor(
        _Domain.AdvancedSecurityOptionsInput, "AdvancedSecurityOptionsInput"
    )
    ClusterConfig = PropertyTypeDescriptor(_Domain.ClusterConfig, "ClusterConfig")
    CognitoOptions = PropertyTypeDescriptor(_Domain.CognitoOptions, "CognitoOptions")
    ColdStorageOptions = PropertyTypeDescriptor(
        _Domain.ColdStorageOptions, "ColdStorageOptions"
    )
    DomainEndpointOptions = PropertyTypeDescriptor(
        _Domain.DomainEndpointOptions, "DomainEndpointOptions"
    )
    EBSOptions = PropertyTypeDescriptor(_Domain.EBSOptions, "EBSOptions")
    EncryptionAtRestOptions = PropertyTypeDescriptor(
        _Domain.EncryptionAtRestOptions, "EncryptionAtRestOptions"
    )
    IAMFederationOptions = PropertyTypeDescriptor(
        _Domain.IAMFederationOptions, "IAMFederationOptions"
    )
    IdentityCenterOptions = PropertyTypeDescriptor(
        _Domain.IdentityCenterOptions, "IdentityCenterOptions"
    )
    Idp = PropertyTypeDescriptor(_Domain.Idp, "Idp")
    JWTOptions = PropertyTypeDescriptor(_Domain.JWTOptions, "JWTOptions")
    LogPublishingOption = PropertyTypeDescriptor(
        _Domain.LogPublishingOption, "LogPublishingOption"
    )
    MasterUserOptions = PropertyTypeDescriptor(
        _Domain.MasterUserOptions, "MasterUserOptions"
    )
    NodeConfig = PropertyTypeDescriptor(_Domain.NodeConfig, "NodeConfig")
    NodeOption = PropertyTypeDescriptor(_Domain.NodeOption, "NodeOption")
    NodeToNodeEncryptionOptions = PropertyTypeDescriptor(
        _Domain.NodeToNodeEncryptionOptions, "NodeToNodeEncryptionOptions"
    )
    OffPeakWindow = PropertyTypeDescriptor(_Domain.OffPeakWindow, "OffPeakWindow")
    OffPeakWindowOptions = PropertyTypeDescriptor(
        _Domain.OffPeakWindowOptions, "OffPeakWindowOptions"
    )
    S3VectorsEngine = PropertyTypeDescriptor(_Domain.S3VectorsEngine, "S3VectorsEngine")
    SAMLOptions = PropertyTypeDescriptor(_Domain.SAMLOptions, "SAMLOptions")
    ServiceSoftwareOptions = PropertyTypeDescriptor(
        _Domain.ServiceSoftwareOptions, "ServiceSoftwareOptions"
    )
    SnapshotOptions = PropertyTypeDescriptor(_Domain.SnapshotOptions, "SnapshotOptions")
    SoftwareUpdateOptions = PropertyTypeDescriptor(
        _Domain.SoftwareUpdateOptions, "SoftwareUpdateOptions"
    )
    VPCOptions = PropertyTypeDescriptor(_Domain.VPCOptions, "VPCOptions")
    WindowStartTime = PropertyTypeDescriptor(_Domain.WindowStartTime, "WindowStartTime")
    ZoneAwarenessConfig = PropertyTypeDescriptor(
